...
(defun file-in-directories-p (filename path-list)
  (let ((path nil)
	(return nil))
...
(if (or (file-in-directories-p "active-buffers.elc" load-path)
	 (file-in-directories-p "active-buffers.el" load-path))
    (progn
      (require 'active-buffers)
